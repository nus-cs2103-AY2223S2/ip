        ...
        try {
            ...
            while (s.hasNext()) {
                ...
                if (line.contains("[T]")) {
                    ...
                    if (line.contains("[X]")) {
                        result.add(new Deadline(line.substring(markIndex).trim(), by, true));
                    } else {
                        result.add(new Deadline(line.substring(unmarkIndex).trim(), by, false));
                    }
                    ...
                    if (line.contains("[X]")) {
                        result.add(new Event(line.substring(markIndex).trim(), from, to, true));
                    } else {
                        result.add(new Event(line.substring(unmarkIndex).trim(), from, to, false));
                    }
                }
            }
            return result;
